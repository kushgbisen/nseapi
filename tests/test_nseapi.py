    ...
    def test_get_holidays_trading(self):
        ...
        with patch("nseapi.helpers.fetch_data_from_nse") as mock_fetch:
            ...
            mock_fetch.return_value = {
                "CM": [
                    {"tradingDate": "22-Jan-2024", "description": "Special Holiday\r"}
                ]
            }

            holidays = get_holidays(holiday_type="trading")

            self.assertIn("CM", holidays)

            self.assertEqual(holidays["CM"][0]["tradingDate"], "22-Jan-2024")

            self.assertEqual(
                holidays["CM"][0]["description"].strip(), "Special Holiday"
            )
